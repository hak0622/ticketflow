...
**처음에는 DB 락으로 충분하다고 생각했지만, 부하 테스트와 장애 로그를 통해 병목과 deadlock을 확인했고, 그 결과 동시성 제어 구조 자체를 다시 설계한 백엔드 프로젝트입니다.**
...
"서버를 더 늘려야 할까, 아니면 지금 구조가 먼저 문제일까?" 라는 질문에서 시작했습니다.

처음에는 `SELECT ... FOR UPDATE` 같은 DB 락만 잘 잡으면 티켓팅 정합성을 충분히 지킬 수 있다고 생각했습니다. 하지만 부하 테스트를 돌려보니 요청 수가 늘어날수록 응답 시간이 급격히 증가했고, Redis 좌석 차감을 붙인 뒤에는 `booked_count` 동기 UPDATE 때문에 deadlock까지 발생했습니다.

그래서 README도 "어떤 기술을 썼는가"보다 "어떤 문제를 보고 왜 판단을 바꿨는가" 순서로 정리했습니다. 이 프로젝트에서 계속 확인한 문제는 아래 네 가지였습니다.
