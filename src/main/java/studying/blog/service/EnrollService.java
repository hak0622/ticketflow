...
public class EnrollService {
    ...
    public EnrollResult enroll(Long lectureId,String userKey){
        ...
        // 3) 중복 신청 방지
        if(enrollmentRepository.existsByLectureIdAndUserKey(lectureId,userKey)){
    ...
    public EnrollResult myEnroll(Long lectureId, String userKey) {
        return enrollmentRepository.findByLectureIdAndUserKey(lectureId, userKey)
                .map(e -> new EnrollResult("ENROLLED", lectureId, e.getLecture().getTitle()))
                .orElseGet(() -> new EnrollResult("NOT_ENROLLED", lectureId, null));
    }

    public record EnrollResult(String status,Long lectureId,String lectureTitle){}

    @Transactional(readOnly = true)
    public EnrollmentResponse getMyEnrollmentDetail(Long lectureId, String userKey) {
        Enrollment enrollment = enrollmentRepository.findByLectureIdAndUserKey(lectureId, userKey)
